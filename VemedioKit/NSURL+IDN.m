...
+ (NSString *)IDNEncodedURLString:(NSString*)urlString
{
    NSString* host = urlString;
    
    // remove scheme
    NSRange hostNameStartRange = [host rangeOfString:@"://"];
    if (hostNameStartRange.location == NSNotFound) {
        hostNameStartRange.location = 0;
    }
    
    host = [host stringByReplacingCharactersInRange:NSMakeRange(0, hostNameStartRange.location+3) withString:@""];
    
    // remove path
    NSRange hostNameEndRange = [host rangeOfString:@"/"];
    if (hostNameEndRange.location != NSNotFound) {
        host = [host stringByReplacingCharactersInRange:NSMakeRange(hostNameEndRange.location, [host length]-hostNameEndRange.location) withString:@""];
    }
    // remove user and pass
    NSRange hostNameUserEndRange = [host rangeOfString:@"@"];
    if (hostNameUserEndRange.location != NSNotFound) {
        host = [host stringByReplacingCharactersInRange:NSMakeRange(0, hostNameUserEndRange.location+1) withString:@""];
    }
    
    NSString* idnHost = [self IDNEncodedHostname:host];
    urlString = [urlString stringByReplacingOccurrencesOfString:host withString:idnHost];
